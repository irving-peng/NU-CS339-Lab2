...
///
/// Shamelessly stolen from erikgrinaker/toydb (https://github.com/erikgrinaker/toydb)
///
...
pub enum Error {
    ...
    /// An IO error.
    IO(String),
    /// Arithmetic integer overflow occurred.
    OverflowError,
    /// A write action was attempted in a read-only transaction.
    ReadOnly,
...
impl std::fmt::Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            Error::OverflowError => write!(f, "integer overflow occurred"),
...
#[macro_export]
macro_rules! errdata {
    ($($args:tt)*) => { $crate::common::Error::InvalidData(format!($($args)*)).into() };
}
...
/// A Rusty-DB Result returning Error.
